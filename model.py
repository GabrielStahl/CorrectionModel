...
class CorrectionUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder
        dec1 = self.dec1(torch.cat([self.upsample(enc9, enc6.size()), enc6], dim=1))
        dec2 = self.dec2(torch.cat([self.upsample(dec1, enc5.size()), enc5], dim=1))
        dec3 = self.dec3(torch.cat([self.upsample(dec2, enc4.size()), enc4], dim=1))
        dec4 = self.dec4(torch.cat([self.upsample(dec3, enc3.size()), enc3], dim=1))
        dec5 = self.dec5(torch.cat([self.upsample(dec4, enc2.size()), enc2], dim=1))
        output = self.dec6(self.upsample(dec5, x.size()))
